...
$RunDataDir = Join-Path $ProjectRoot "runData"
...
$VintageStoryExe = Join-Path $env:VINTAGE_STORY "VintagestoryServer.exe"

if (-not (Test-Path $VintageStoryExe)) {
    Write-Host "Error: VintagestoryServer.exe not found at: $VintageStoryExe" -ForegroundColor Red
    exit 1
}
